...
// 有时也将这两个函数称为散布读（scatter read）和聚集写（gather write）。
    ...
    ssize_t Buffer::readFd(int fd, int *saveErrno) {
        // 栈额外空间，用于从套接字往出读时
        // 当buffer_暂时不够用时暂存数据，待buffer_重新分配足够空间后，在把数据交换给buffer_。
        ...
        */
        ...
        // 使用iovec分配两个连续的缓冲区
        // 这是Buffer底层缓冲区剩余的可写空间大小 不一定能完全存储从fd读出的数据
        struct iovec vec[2];
        // 后面调用readv函数会改写，所以提前拷出来
...
// outputBuffer_.writeFd标示将数据写入到outputBuffer_中
// 从readerIndex_开始，可以写readableBytes()个字节
